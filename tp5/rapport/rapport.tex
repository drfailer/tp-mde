...
toujours la même séquence de nombre (avec la seed fournie par Makoto Matsumoto).
Pour pouvoir paralléliser les calculs, on doit pouvoir lancer le générateur à
partir d'un certain état et pour ce faire on utilisera des fichiers de statuts.
...
\begin{listing}[ht!]
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=LightGray,fontsize=\normalsize,linenos]{bash}
  git submodule init
...
utilisant CMake (utiliser un répertoire \lstinline{build/} à la racine du
projet).

À noter que pour compiler la bibliothèque CLHEP, le script \lstinline{build.sh}
utilise \lstinline{make} avec l'option \lstinline{-j} pour paralléliser le
compilation. Cette option est pratique car elle permet de compiler les fichiers
plus rapidement, cependant, elle peut être consommatrice en ressource. En effet,
ici, on ne spécifie pas le nombre de threads donc \lstinline{make} utilise
...
\lstinline{generateStatusFiles}. On peut configurer le nombre de fichiers à
